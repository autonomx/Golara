...
## Phase 32 closeout criteria

Repo-side Phase 32 can be considered complete when the repository contains webhook receipt, signature verification, idempotent persistence, trusted state transitions, durable settlement reconciliation storage, admin settlement/alert visibility, deploy-readiness blockers, runbooks, launch checklist links, evidence templates, and wired guards.

Target-environment Phase 32 can be considered validated only after the settlement migration is applied in the intended database, provider-generated Stripe/ZarinPal webhook cases are captured, duplicate replay and invalid-signature cases are recorded, admin settlement/alert pages are reviewed, deploy-readiness output is captured, and the validation evidence template is filled in.

Out of scope for Phase 32 closeout: provider-backed refunds/voids, provider dashboard settlement imports, actual outbound alert delivery, real notification providers, and durable outbound webhook workers. Those remain Phase 33, Phase 34, and Phase 35 work.
