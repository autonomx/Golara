...
1. If provider dashboard access is available, run the Phase 32 validation sequence and fill in the evidence template.
2. If provider/environment access is not available, treat repo-side Phase 32 as ready for operator validation and avoid adding more repository-only webhook claims.
