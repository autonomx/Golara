...
Payment operation navigation consistency is source-guarded across the landing, provider-readiness, history, preview, settlement entry page, admin-navigation documentation, operator runbook, and go/no-go checklist. The guard checks required read-only links and evidence-gate language while continuing to block provider calls, adapter execution, default fetch, Prisma access, live provider endpoint URLs, buttons, click handlers, and order/payment mutation SQL in those surfaces.
...
- keep provider-operation readiness route-core helpers, panels, pages, landing pages, settlement navigation, cross-links, admin-navigation documentation, operator runbooks, and go/no-go checklists read-only and free of execution controls, click handlers, provider calls, direct Prisma access, and mutation paths;
