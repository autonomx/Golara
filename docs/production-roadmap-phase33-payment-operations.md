...
`buildPaymentOperationProviderReadinessSummary` can aggregate multiple provider diagnostics into ready, needs-operator-evidence, manual-review, and unavailable counts without enabling execution.
...
1. Add a compact read-only admin/provider diagnostics panel or route section for `buildPaymentOperationProviderReadinessSummary` after confirming the admin UX location.
