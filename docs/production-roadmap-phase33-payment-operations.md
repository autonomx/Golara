...
The payment settlement admin page now surfaces read-only links to `/admin/payments/operations/providers`, `/admin/payments/operations/history`, and `/admin/payments/operations/preview` so operators can reach Phase 33 diagnostics from the existing settlement entry point. The surfacing is navigation-only and does not call provider adapters, submit refunds or voids, call fetch, use direct Prisma access, mutate orders/payments, release inventory/capacity, render buttons, or attach click handlers.
