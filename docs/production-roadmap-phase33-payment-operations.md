...
Payment operation navigation consistency is source-guarded across the landing, provider-readiness, history, preview, settlement entry page, admin-navigation documentation, operator runbook, go/no-go checklist, and provider-readiness evidence example. The guard checks required read-only links, evidence-gate language, credential-source-name-only evidence examples, and no-go language while continuing to block provider calls, adapter execution, default fetch, Prisma access, live provider endpoint URLs, buttons, click handlers, and order/payment mutation SQL in those surfaces.
