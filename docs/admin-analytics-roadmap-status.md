...
- Scheduled report preview, owner-approved configuration-plan foundation, inactive storage schema, Prisma mapping contract, checked Prisma schema fragment, metadata-only read-model foundation, repository-read query-plan contract, and read-adapter foundation that reuse the selected analytics range and aggregate Business/Site CSV paths.
...
- Scheduled report `schema.prisma` model application, generated Prisma client access, active repository wiring, repository writes, read endpoints, delivery channel execution, retry visibility, owner approval recording, global disable controls, and owner management UI.
...
The scheduled report foundation now includes preview metadata, configuration-plan metadata, an inactive `AdminAnalyticsScheduledReport` storage schema, a Prisma mapping contract, a checked Prisma schema fragment, a metadata-only read-model foundation, a repository-read query-plan contract, and a read-adapter foundation. It defines weekly and monthly owner report options, selected-range metadata, aggregate Business/Site CSV paths, owner-approval requirements, metadata-only future schedule fields, a future model block, a checked standalone schema fragment, required defaults and indexes, safe DTO normalization for future table rows, required future read filters, safe select fields, and activation blockers without saving active schedules, sending reports, creating timers, enabling generated Prisma client access, wiring Prisma repository reads, or running background jobs.

The checked schema fragment exactly matches the guarded model block but is not yet applied to `prisma/schema.prisma`. The read model keeps `activeForOperators=false` and `deliveryReady=false` even when future approval, active, delivery, and dry-run fields are present. The repository-read contract defines owner-approved, active, delivery-disabled future filters. The read adapter builds and applies those same query args to an injected reader for validation, while active Prisma repository wiring remains disabled. Actual scheduled delivery remains pending until the main Prisma schema application, generated Prisma client access, repository wiring, owner approval recording, dry-run evidence, delivery configuration, retry/failure visibility, disable controls, and management UI are designed and validated.
...
12. Scheduled report previews, config plans, inactive storage schema, Prisma mapping contract, checked schema fragment, read model, repository-read contract, and read adapter preserve the selected range and aggregate Business/Site CSV paths without enabling delivery.
