...
- Scheduled report generated Prisma client usage in application contracts, active repository wiring, repository writes, read endpoints, delivery channel execution, retry visibility, owner approval recording, global disable controls, schedule activation, scheduler/timer/background execution, and owner management UI.
...
The scheduled report foundation now includes preview metadata, configuration-plan metadata, an inactive `AdminAnalyticsScheduledReport` storage schema, an applied `schema.prisma` model mapping, a checked Prisma schema fragment, a metadata-only read-model foundation, a repository-read query-plan contract, and a read-adapter foundation. It defines weekly and monthly owner report options, selected-range metadata, aggregate Business/Site CSV paths, owner-approval requirements, metadata-only future schedule fields, an applied model block, a checked standalone schema fragment, required defaults and indexes, safe DTO normalization for future table rows, required future read filters, safe select fields, and activation blockers without saving active schedules, sending reports, creating timers, enabling application use of the generated Prisma client, wiring Prisma repository reads, or running background jobs.

The `schema.prisma` model mapping is applied and exactly matches the checked fragment and guarded model block. The read model keeps `activeForOperators=false` and `deliveryReady=false` even when future approval, active, delivery, and dry-run fields are present. The repository-read contract defines owner-approved, active, delivery-disabled future filters. The read adapter builds and applies those same query args to an injected reader for validation, while application use of the generated Prisma client and active Prisma repository wiring remain disabled. Actual scheduled delivery remains pending until generated client usage, repository wiring, owner approval recording, dry-run evidence, delivery configuration, retry/failure visibility, disable controls, schedule activation, scheduler/timer/background execution, and management UI are designed and validated.
