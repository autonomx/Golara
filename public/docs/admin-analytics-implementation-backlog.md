...
- Production validation evidence for custom ranges, exports, aggregate cohort panels, retention preview, scheduled report storage/read-model/repository-contract, saved view storage/read-model, and dashboard group headers.
...
Scheduled report config plans, inactive storage schema, metadata-only read model, and repository-read contract preserve the selected range and aggregate CSV paths. They require owner approval but do not activate schedules, expose read routes, run active repository reads, or send reports yet.
