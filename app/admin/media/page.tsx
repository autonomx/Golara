...
function MediaCategorySelect({ defaultValue = 'product', disabled = false, compact = false, locale }: { defaultValue?: string | null; disabled?: boolean; compact?: boolean; locale?: SupportedLocale | string | null }) {
    ...
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-stone-200 bg-white lg:flex lg:flex-col">
      <div className="border-b border-stone-200 px-5 py-5">
        <Link href="/admin" className="flex items-center gap-3">
          ...
          <span><span className="block text-sm font-bold text-stone-950">Golara</span><span className="block text-xs font-medium text-stone-500">{t('Operations console')}</span></span>
        </Link>
      </div>
        ...
        <div className="grid gap-5">
          {sidebarSections.map((section) => (
            <div key={section.label}>
              <p className="px-3 pb-2 text-[11px] font-bold uppercase tracking-[0.16em] text-stone-400">{t(section.label)}</p>
      ...
      <header className="sticky top-0 z-20 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div className="flex min-h-16 flex-wrap items-center justify-between gap-3 px-4 py-3 lg:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><h1 className="text-lg font-bold text-stone-950">{t('Media library')}</h1><p className="truncate text-xs font-medium text-stone-500">{t('Manage image uploads, URL media, category tags, and usage.')}</p></div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <LanguageSwitcher locale={locale} returnTo="/admin/media" />
            <span className="rounded-md border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs font-semibold text-stone-700">{media.length} {t('media')}</span>
          </div>
        </div>
      </header>
        ...
        <section className="min-w-0 rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-stone-500">{t('Admin / Catalog')}</p>
              <h2 className="mt-1 text-2xl font-bold text-stone-950">{t('Media library')}</h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">{t('Manage image uploads, URL media, category tags, and usage.')}</p>
            </div>
            {!authenticated ? <Link href="/admin/login" className={buttonClass}>{authConfigured ? t('Sign in') : t('Configure auth')}</Link> : null}
          </div>
        </section>
          ...
          <details className="rounded-lg border border-rosewood/10 bg-cream p-5">
            ...
            <div className="mt-5 grid gap-6 lg:grid-cols-2">
              <form action={createMediaFromUrlAction} className="grid gap-4 rounded-lg border border-rosewood/10 bg-white p-5 shadow-sm">
                ...
                <MediaCategorySelect disabled={disabled} locale={locale} />
                <label className="grid gap-2 text-sm font-semibold text-rosewood">{t('Image URL')}<input className={inputClass} name="url" placeholder="https://..." disabled={disabled} required /></label>
                <label className="grid gap-2 text-sm font-semibold text-rosewood">{t('Alt text')}<input className={inputClass} name="alt" placeholder={t('Blush rose bouquet')} disabled={disabled} required /></label>
                <button className={buttonClass} type="submit" disabled={disabled}>{t('Add media')}</button>
              </form>
              <form action={uploadMediaAction} className="grid gap-4 rounded-lg border border-rosewood/10 bg-white p-5 shadow-sm">
                ...
                <label className="grid gap-2 text-sm font-semibold text-rosewood">{t('Alt text')}<input className={inputClass} name="alt" placeholder={t('Optional descriptive text')} disabled={disabled} /></label>
                <button className={buttonClass} type="submit" disabled={disabled}>{t('Upload image')}</button>
              </form>
            </div>
          </details>
          ...
          <div className="mt-8 max-h-[760px] overflow-y-auto rounded-lg border border-rosewood/10 bg-white">
            <table className="w-full table-fixed border-collapse text-left text-sm">
              <thead className="sticky top-0 z-[1] bg-cream text-xs font-semibold uppercase tracking-[0.16em] text-rosewood/70">
                <tr>
                  <th className="w-[28%] px-3 py-3">{t('Image')}</th>
                  <th className="w-[20%] px-3 py-3">{t('Category')}</th>
                  <th className="w-[13%] px-3 py-3">{t('Source')}</th>
                  <th className="w-[21%] px-3 py-3">URL</th>
                  <th className="w-[18%] px-3 py-3">{t('Actions')}</th>
                </tr>
              </thead>
              <tbody>
                {media.map((item) => {
                    ...
                    <tr key={item.id ?? safeUrl ?? item.alt} className="border-t border-rosewood/10 align-top">
                      <td className="px-3 py-4">
                        <div className="flex min-w-0 gap-3">
                          ...
                          <div className="min-w-0"><div className="truncate font-semibold text-rosewood">{item.alt}</div><div className="mt-1 truncate text-xs text-stone-500">{item.createdAt ? item.createdAt.toLocaleDateString('en-CA') : t('Seed or static asset')}</div></div>
                        </div>
                      </td>
                      <td className="px-3 py-4">
                        {item.id ? (
                          <form action={updateMediaCategoryAction.bind(null, item.id)} className="grid min-w-0 gap-2">
                            <MediaCategorySelect defaultValue={item.mediaCategory} disabled={disabled} compact locale={locale} />
                            <button type="submit" className={secondaryButtonClass} disabled={disabled}>{t('Save')}</button>
                          </form>
                        ...
                        {item.id ? (
                          <details className="min-w-0">
                            ...
                            <form action={updateMediaAction.bind(null, item.id)} className="mt-4 grid min-w-0 gap-3 rounded-lg border border-rosewood/10 bg-[#fffdfb] p-3">
                              <label className="grid min-w-0 gap-2 text-xs font-semibold text-rosewood">{t('Image URL')}<input className={compactInputClass} name="url" defaultValue={safeUrl ?? ''} disabled={disabled} required /></label>
                              <label className="grid min-w-0 gap-2 text-xs font-semibold text-rosewood">{t('Alt text')}<input className={compactInputClass} name="alt" defaultValue={item.alt} disabled={disabled} required /></label>
                              <MediaCategorySelect defaultValue={item.mediaCategory} disabled={disabled} compact locale={locale} />
                              <button className={buttonClass} type="submit" disabled={disabled}>{t('Update')}</button>
                            </form>
                          </details>
