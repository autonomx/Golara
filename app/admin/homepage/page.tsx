...
import { createAdminHomeTranslator } from '@/lib/localization/admin-home-copy';
...
function AdminSidebar({ authenticated, authConfigured, locale }: { authenticated: boolean; authConfigured: boolean; locale?: SupportedLocale | string | null }) {
  const t = createAdminTranslator(locale);
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-stone-200 bg-white lg:flex lg:flex-col">
      <div className="border-b border-stone-200 px-5 py-5"><Link href="/admin" className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-md bg-rosewood font-display text-xl text-white">G</span><span><span className="block text-sm font-bold text-stone-950">Golara</span><span className="block text-xs font-medium text-stone-500">{t('Operations console')}</span></span></Link></div>
      <nav aria-label={t('Admin navigation')} className="flex-1 overflow-y-auto px-3 py-4"><div className="grid gap-5">{sidebarSections.map((section) => <div key={section.label}><p className="px-3 pb-2 text-[11px] font-bold uppercase tracking-[0.16em] text-stone-400">{t(section.label)}</p><div className="grid gap-1">{section.items.map((item) => { const Icon = item.icon; const active = item.key === 'homepage'; return <Link key={item.href} href={item.href} aria-current={active ? 'page' : undefined} className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-semibold transition ${active ? 'bg-rosewood text-white shadow-sm' : 'text-stone-700 hover:bg-stone-100 hover:text-stone-950'}`}><Icon aria-hidden="true" className="h-4 w-4" />{t(item.label)}</Link>; })}</div></div>)}</div></nav>
      <div className="border-t border-stone-200 p-4">{authenticated ? <div className="rounded-md border border-stone-200 bg-stone-50 p-3"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">{t('Signed in')}</p><p className="mt-1 text-sm font-semibold text-stone-900">{t('Admin')}</p></div> : <Link href="/admin/login" className="flex items-center justify-center gap-2 rounded-md bg-rosewood px-4 py-2.5 text-sm font-semibold text-white"><LogIn aria-hidden="true" className="h-4 w-4" />{authConfigured ? t('Sign in') : t('Configure auth')}</Link>}</div>
    </aside>
  );
}
...
function HomepageCategoryManager({ categories, homepageCategories, media, disabled, occasionPage, locale }: { categories: Category[]; homepageCategories: Category[]; media: MediaItem[]; disabled: boolean; occasionPage: number; locale?: SupportedLocale | string | null }) {
  const copy = createAdminHomeTranslator(locale);
  ...
  const range = pagedCategories.length ? `${start + 1}-${start + pagedCategories.length}` : '0';

  return (
    <section className={sectionClass}>
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-olive">{copy('occasionsLabel')}</p><h3 className="mt-1 font-display text-3xl text-rosewood">{copy('occasionsTitle')}</h3><p className="mt-2 max-w-3xl text-sm text-stone-600">{copy('showing')} {range} {copy('of')} {homepageCategories.length}. {copy('occasionsHelp')}</p></div>
        <div className="flex items-center gap-2 text-sm font-semibold"><Link aria-disabled={currentPage <= 1} href={pageHref('occasionPage', Math.max(1, currentPage - 1))} className={`rounded-md border px-3 py-2 ${currentPage <= 1 ? 'pointer-events-none border-stone-200 text-stone-300' : 'border-rosewood/20 text-rosewood'}`}>{copy('previous')}</Link><span className="rounded-md border border-stone-200 bg-stone-50 px-3 py-2 text-stone-700">{copy('page')} {currentPage} {copy('of')} {pageCount}</span><Link aria-disabled={currentPage >= pageCount} href={pageHref('occasionPage', Math.min(pageCount, currentPage + 1))} className={`rounded-md border px-3 py-2 ${currentPage >= pageCount ? 'pointer-events-none border-stone-200 text-stone-300' : 'border-rosewood/20 text-rosewood'}`}>{copy('next')}</Link></div>
      </div>
...
function FeaturedPicksManager({ products, disabled, featuredPage, locale }: { products: Product[]; disabled: boolean; featuredPage: number; locale?: SupportedLocale | string | null }) {
  const copy = createAdminHomeTranslator(locale);
  ...
  const range = pagedProducts.length ? `${start + 1}-${start + pagedProducts.length}` : '0';

  return (
    <section className={sectionClass}>
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-olive">{copy('featuredLabel')}</p><h3 className="mt-1 font-display text-3xl text-rosewood">{copy('featuredTitle')}</h3><p className="mt-2 max-w-3xl text-sm text-stone-600">{copy('showing')} {range} {copy('of')} {featuredProducts.length}. {copy('featuredHelp')}</p></div>
        <div className="flex items-center gap-2 text-sm font-semibold"><Link aria-disabled={currentPage <= 1} href={pageHref('featuredPage', Math.max(1, currentPage - 1))} className={`rounded-md border px-3 py-2 ${currentPage <= 1 ? 'pointer-events-none border-stone-200 text-stone-300' : 'border-rosewood/20 text-rosewood'}`}>{copy('previous')}</Link><span className="rounded-md border border-stone-200 bg-stone-50 px-3 py-2 text-stone-700">{copy('page')} {currentPage} {copy('of')} {pageCount}</span><Link aria-disabled={currentPage >= pageCount} href={pageHref('featuredPage', Math.min(pageCount, currentPage + 1))} className={`rounded-md border px-3 py-2 ${currentPage >= pageCount ? 'pointer-events-none border-stone-200 text-stone-300' : 'border-rosewood/20 text-rosewood'}`}>{copy('next')}</Link></div>
      </div>
...
export default async function AdminHomepagePage({ searchParams }: { searchParams: Promise<{ status?: string; occasionPage?: string; featuredPage?: string }> }) {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-stone-50 lg:pl-72" dir={getStorefrontCopyDirection(locale)}>
      <AdminSidebar authenticated={authenticated} authConfigured={authConfigured} locale={locale} />
      <header className="sticky top-0 z-20 border-b border-stone-200 bg-white/95 backdrop-blur"><div className="flex min-h-16 flex-wrap items-center justify-between gap-3 px-4 py-3 lg:px-6"><div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-md border border-stone-200 bg-stone-50 text-stone-700"><Home className="h-4 w-4" /></span><div className="min-w-0"><h1 className="text-lg font-bold text-stone-950">{t('Homepage')}</h1><p className="truncate text-xs font-medium text-stone-500">{t('Edit hero, calls to action, homepage sections, footer copy, displayed occasions, and featured picks.')}</p></div></div><div className="flex flex-wrap items-center gap-2"><LanguageSwitcher locale={locale} returnTo="/admin/homepage" /><Link href="/" className="rounded-md border border-rosewood/15 bg-white px-4 py-2 text-sm font-semibold text-rosewood">{t('View storefront')}</Link></div></div></header>
      <section className="grid min-w-0 gap-6 px-4 py-6 lg:px-6">
        {status ? <div className="rounded-lg border border-olive/20 bg-white p-4 text-sm font-semibold text-olive shadow-sm">{t(status === 'homepage-category-removed' ? 'Homepage occasion tile removed.' : status === 'homepage-category-added' ? 'Homepage occasion tile added.' : status === 'homepage-category-updated' ? 'Homepage occasion tile saved.' : status === 'homepage-featured-added' ? 'Featured pick added.' : status === 'homepage-featured-removed' ? 'Featured pick removed.' : status === 'homepage-featured-updated' ? 'Featured pick saved.' : 'Homepage saved.')}</div> : null}
        <section className={sectionClass}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-stone-500">{t('Admin / Content')}</p><h2 className="mt-1 text-2xl font-bold text-stone-950">{t('Homepage editor')}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">{t('Control the live homepage hero, CTAs, trust chips, occasion section copy, footer text, every occasion tile, and featured pick shown on the homepage.')}</p></div>{!authenticated ? <Link href="/admin/login" className={buttonClass}>{authConfigured ? t('Sign in to edit') : t('Configure auth')}</Link> : null}</div></section>
        ...
        <HomepageCategoryManager categories={categories} homepageCategories={homepageCategories} media={media} disabled={disabled} occasionPage={parsedOccasionPage} locale={locale} />
        <FeaturedPicksManager products={products} disabled={disabled} featuredPage={parsedFeaturedPage} locale={locale} />
      </section>
    </main>
  );
}
