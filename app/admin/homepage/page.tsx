...
import { getHomepageContent, listCategories, listHomepageCategories, listMedia } from '@/lib/cms/catalog-repository';
...
import { updateHomepageCategoryTileAction } from '@/app/admin/homepage/category-actions';
...
function CategoryTileEditor({ category, categories, media, disabled }: { category: Category; categories: Category[]; media: MediaItem[]; disabled: boolean }) {
  ...
  return (
    <details className="rounded-lg border border-rosewood/10 bg-[#fffdfb] p-4 shadow-sm">
      ...
      <form action={updateHomepageCategoryTileAction.bind(null, category.id ?? '')} className="mt-5 grid gap-4">
...
export default async function AdminHomepagePage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  const [{ status }, homepage, media, categories, homepageCategories, authenticated] = await Promise.all([searchParams, getHomepageContent(), listMedia(), listCategories(), listHomepageCategories(), isAdminAuthenticated()]);
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-stone-50 lg:pl-72">
      ...
      <section className="grid min-w-0 gap-6 px-4 py-6 lg:px-6">
        {status === 'homepage-updated' || status === 'homepage-category-updated' ? <div className="rounded-lg border border-olive/20 bg-white p-4 text-sm font-semibold text-olive shadow-sm">{status === 'homepage-category-updated' ? 'Homepage occasion tile saved.' : 'Homepage saved.'}</div> : null}
        ...
        <section className={sectionClass}><div className="mb-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-olive">Displayed occasions</p><h3 className="mt-1 font-display text-3xl text-rosewood">Homepage occasion tiles</h3><p className="mt-2 max-w-3xl text-sm text-stone-600">These are the occasion/category cards shown in the homepage section. Edit the displayed title, eyebrow, description, image, visibility, active state, and order here.</p></div><div className="grid gap-4">{homepageCategories.map((category) => <CategoryTileEditor key={category.id ?? category.slug} category={category} categories={categories} media={media} disabled={disabled} />)}</div></section>
      </section>
    </main>
  );
}
