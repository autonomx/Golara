...
import { BadgePercent, FileText, LayoutDashboard, Package, Settings, ShoppingBag, Users } from 'lucide-react';
...
type AdminModuleHeaderCopy = { eyebrow: string; title: string; description: string; action?: { href: string; label: string } };
...
const navPathByKey: Record<AdminNavKey, string> = {
  overview: '/admin',
...
const moduleHeaders: Record<string, AdminModuleHeaderCopy> = {
  ...
  'payment-methods': { eyebrow: 'Admin / Payments', title: 'Payment methods', description: 'Manage checkout payment methods and admin controls.' },
...
const homepageFormCopy = {
  ...
  secondaryUrl: 'Secondary CTA URL',
...
function moduleHeader(activeTab: AdminTab, activeNavKey: AdminNavKey): AdminModuleHeaderCopy {
  return moduleHeaders[activeNavKey] ?? moduleHeaders[activeTab] ?? moduleHeaders.overview;
}

function AdminModuleHeader({ header }: { header: AdminModuleHeaderCopy }) {
  return (
...
function adminPathForNavKey(activeNavKey: AdminNavKey, activeTab: AdminTab) {
  return navPathByKey[activeNavKey] ?? tabHref(activeTab);
}

function adminReturnPath(activeNavKey: AdminNavKey, activeTab: AdminTab, params: AdminSearchParams) {
...
function isAdminNavKey(value: string): value is AdminNavKey {
  return value in navPathByKey;
}

export async function AdminConsolePage({ searchParams, forcedTab, catalogSection = 'all', salesSection = 'all', contentSection = 'all', overviewSection = 'all', activeNavKey }: { searchParams: Promise<AdminSearchParams>; forcedTab?: AdminTab; catalogSection?: CatalogSection; salesSection?: SalesSection; contentSection?: ContentSection; overviewSection?: OverviewSection; activeNavKey?: AdminNavKey }) {
  ...
  const fallbackNavKey = isAdminNavKey(activeTab) ? activeTab : 'overview';
  const resolvedActiveNavKey = activeNavKey ?? fallbackNavKey;
  ...
  return (
    <AdminPageShell activeTab={activeTab} activeNavKey={resolvedActiveNavKey} authenticated={authenticated} authConfigured={authConfigured} adminLabel={adminIdentity?.label ?? adminIdentity?.email} locale={locale} returnTo={languageReturnTo} productCount={products.length} categoryCount={categories.length} mediaCount={media.length}>
      ...
      {activeTab === 'discounts' ? <AdminModulePlaceholder eyebrow={discountsPlaceholder.eyebrow} title={discountsPlaceholder.title} body={discountsPlaceholder.body} items={[...discountsPlaceholder.items]} locale={locale} /> : null}
