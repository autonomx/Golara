        ...
        <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-bold text-stone-950">{t('Payment operation history')}</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
                {t('Read-only Phase 33 operation history entry point for persisted refund and void records. Provide an orderId query parameter to review migration-gated operation records without creating records, calling providers, or mutating orders/payments.')}
              </p>
            </div>
          ...
          <section className="rounded-lg border border-amber-200 bg-amber-50 p-5 text-amber-950 shadow-sm">
            ...
            <dl className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="rounded-lg border border-amber-200 bg-white/70 p-3">
                <dt className="text-xs font-semibold uppercase tracking-[0.14em]">{t('Flag')}</dt>
                <dd className="mt-2 break-words text-sm font-semibold">{historyResult.body.migrationStatus.flagName}</dd>
              </div>
              <div className="rounded-lg border border-amber-200 bg-white/70 p-3">
                <dt className="text-xs font-semibold uppercase tracking-[0.14em]">{t('Migration')}</dt>
