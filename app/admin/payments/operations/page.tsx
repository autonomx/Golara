        ...
        <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-bold text-stone-950">Payment operations</h1>
          ...
          <div className="mt-4 rounded-lg bg-stone-50 p-3 text-sm text-stone-600">
            {authConfigured ? authenticated ? `Signed in as ${identity.label ?? identity.email ?? 'admin'}.` : 'Admin authentication is required to view payment operation diagnostics.' : 'Admin authentication is not configured yet.'}
          </div>
          ...
          <section className="grid gap-3 md:grid-cols-3">
            {operationLinks.map((link) => (
              <Link key={link.href} href={link.href} className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm transition hover:border-stone-300">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-stone-500">Read-only</p>
                <h2 className="mt-2 text-xl font-bold text-stone-950">{link.label}</h2>
