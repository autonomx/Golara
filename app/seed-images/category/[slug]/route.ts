...
function sceneShape(slug: string, dark: string, main: string) {
  if (slug === 'cake-balloon') {
    return `<circle cx="420" cy="360" r="68" fill="${main}" opacity=".7"/><circle cx="495" cy="320" r="54" fill="#fff" opacity=".82"/><line x1="420" y1="428" x2="520" y2="680" stroke="${dark}" stroke-width="4" opacity=".35"/><rect x="455" y="690" width="300" height="170" rx="34" fill="#fff" opacity=".94"/><rect x="490" y="650" width="230" height="70" rx="28" fill="${main}" opacity=".28"/>`;
  }
  if (slug === 'weddings' || slug === 'proposal' || slug === 'ceremony-design') {
    return `<path d="M345 860 C430 655 520 560 600 520 C680 560 770 655 855 860 Z" fill="#fff" opacity=".86"/><path d="M410 845 C470 690 540 610 600 585 C660 610 730 690 790 845 Z" fill="${main}" opacity=".14"/><circle cx="600" cy="455" r="76" fill="#fff" opacity=".9"/>`;
  }
  if (slug === 'pots') {
    return `<path d="M420 610 H780 L720 900 H480 Z" fill="#fff" opacity=".94"/><rect x="460" y="580" width="280" height="40" rx="20" fill="${dark}" opacity=".18"/>`;
  }
  if (slug === 'royal') {
    return `<rect x="385" y="560" width="430" height="310" rx="44" fill="#fff" opacity=".9"/><path d="M450 560 L505 465 L560 560 L615 455 L670 560 L725 465 L780 560 Z" fill="${dark}" opacity=".25"/><rect x="455" y="800" width="290" height="78" rx="28" fill="${dark}" opacity=".66"/>`;
  }
