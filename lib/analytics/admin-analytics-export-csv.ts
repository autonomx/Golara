...
import type { CategorySalesAnalyticsSummary } from './category-sales-analytics';
import type { OrderRevenueSummary } from './order-revenue-summary';
import type { ProductSalesAnalyticsSummary } from './product-sales-analytics';
import type { SiteAnalyticsSummary } from './site-analytics-summary';

export type AnalyticsExportReport = 'business' | 'site';
...
export function normalizeAnalyticsExportReport(value: string | null): AnalyticsExportReport {
...
export function analyticsFilename(report: AnalyticsExportReport, range: AdminAnalyticsResolvedRange) {
...
export function buildBusinessAnalyticsCsv(
  range: AdminAnalyticsResolvedRange,
  summary: OrderRevenueSummary,
  ...
  for (const [status, count] of Object.entries(summary.byStatus)) rows.push(['business', 'orders_by_status', 'order_count', status, count, '', '']);
...
export function buildSiteAnalyticsCsv(range: AdminAnalyticsResolvedRange, summary: SiteAnalyticsSummary) {
  const rows: unknown[][] = [
    ['report', 'section', 'metric', 'label', 'value', 'currency', 'notes'],
  ...
  for (const point of summary.recentDaily) rows.push(['site', 'daily', 'event_count', point.date, point.eventCount, '', '']);
