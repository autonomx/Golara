...
export function formatAdminFailedPaymentAlertTitle(alert: { kind: string; status: string }, locale?: SupportedLocale | string | null) {
...
export function formatAdminFailedPaymentNotificationDetail(
  ...
  return alert.detail
    .replace(/\bto\b/g, getAdminFailedPaymentAlertCopy('to', locale))
    .replace(/\battempt\b/g, getAdminFailedPaymentAlertCopy('attempt', locale));
}
