import { updatePaymentMethodSettingAction } from '@/app/admin/settings/payment-method-actions';
import {
          ...
          return (
            <form key={method.key} action={updatePaymentMethodSettingAction} className="grid gap-4 rounded-md border border-stone-200 bg-stone-50 p-4">
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_0.8fr_0.5fr]">
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Label<input className={inputClass} name="label" defaultValue={method.label} disabled={!databaseReady} /></label>
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Provider key<input className={inputClass} name="providerKey" defaultValue={method.providerKey} disabled={!databaseReady} /></label>
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Sort<input className={inputClass} name="sortOrder" type="number" defaultValue={method.sortOrder} disabled={!databaseReady} /></label>
              </div>
              <label className="grid gap-2 text-sm font-semibold text-stone-800">Description<input className={inputClass} name="description" defaultValue={method.description ?? ''} disabled={!databaseReady} /></label>
              <div className="grid gap-3 md:grid-cols-4">
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Method type<select className={inputClass} name="methodType" defaultValue={method.methodType} disabled={!databaseReady}><OptionList values={PAYMENT_METHOD_TYPES} /></select></label>
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Settlement<select className={inputClass} name="settlementMode" defaultValue={method.settlementMode} disabled={!databaseReady}><OptionList values={PAYMENT_METHOD_SETTLEMENT_MODES} /></select></label>
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Capture<select className={inputClass} name="captureMode" defaultValue={method.captureMode} disabled={!databaseReady}><OptionList values={PAYMENT_METHOD_CAPTURE_MODES} /></select></label>
                <label className="grid gap-2 text-sm font-semibold text-stone-800">Currency<input className={inputClass} name="currency" defaultValue={method.currency} disabled={!databaseReady} /></label>
              </div>
              ...
              <button className="w-fit rounded-full bg-rosewood px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-rosewood/20 disabled:cursor-not-allowed disabled:bg-stone-300 disabled:shadow-none" type="submit" disabled={!databaseReady}>Save payment method</button>
            </form>
          );
        })}
      </div>
