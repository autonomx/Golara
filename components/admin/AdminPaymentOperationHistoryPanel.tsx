...
export function AdminPaymentOperationHistoryPanel({ view, locale }: { view: PaymentOperationHistoryView; locale?: SupportedLocale | string | null }) {
    ...
    <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-bold text-stone-950">{t(view.heading)}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">{t(view.summary)}</p>
        </div>
      ...
      <dl className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {view.summaryRows.map((summary) => (
          <div key={summary.label} className="rounded-lg border border-stone-200 bg-stone-50 p-3">
            <dt className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">{t(summary.label)}</dt>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {view.filterLabels.map((filter) => (
          <span key={filter.label} className="rounded-full border border-stone-200 bg-white px-3 py-1 text-xs font-semibold text-stone-700">
            {t(filter.label)}: {t(String(filter.value))}
          </span>
        ))}
      </div>
      ...
      <dl className="mt-4 grid gap-3 md:grid-cols-3">
        {view.facetLabels.map((facet) => (
          <div key={facet.label} className="rounded-lg border border-stone-200 bg-white p-3">
            <dt className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">{t(facet.label)}</dt>
      ...
      {view.status === 'empty' ? (
        <div className="mt-5 rounded-lg border border-dashed border-stone-300 bg-stone-50 p-4 text-sm text-stone-600">
          {t('No payment operation records have been persisted for this order. This read-only panel only displays rows after the target environment confirms the migration gate.')}
        </div>
      ) : (
            ...
            <article key={row.id} className="rounded-lg border border-stone-200 bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-stone-500">{t('Requested by')} {row.requestedByLabel}</p>
                </div>
              ...
              <dl className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                {row.detailRows.map((detail) => (
                  <div key={detail.label} className="rounded-lg border border-stone-200 bg-stone-50 p-3">
                    <dt className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">{t(detail.label)}</dt>
              ...
              <p className="mt-4 text-xs leading-5 text-stone-500">
                {t('Created')} {row.createdAtLabel}. {t('Last updated')} {row.updatedAtLabel}. {t('This panel does not render refund or void execution controls.')}
              </p>
            </article>
          ))}
        </div>
