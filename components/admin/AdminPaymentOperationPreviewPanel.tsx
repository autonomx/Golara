...
export function AdminPaymentOperationPreviewPanel({ result, locale }: { result: PaymentOperationPreviewRouteResult; locale?: SupportedLocale | string | null }) {
  ...
  return (
    <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-stone-500">{t('Payments')}</p>
          <h2 className="mt-1 text-2xl font-bold text-stone-950">{t('Payment operation preview')}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
            {t('Read-only refund and void preview for operator review before any persistence, provider execution, order mutation, or payment attempt mutation is added.')}
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="rounded-lg border border-stone-200 bg-stone-50 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">{t('Preview summary')}</p>
        ...
        <div className="rounded-lg border border-stone-200 bg-white p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">{t('Read-only action')}</p>
          <p className="mt-2 text-lg font-bold text-stone-950">{preview.actionLabel}</p>
          <p className="mt-2 text-sm leading-6 text-stone-600">{t('This panel does not render a refund or void execution button.')}</p>
        </div>
      </div>
      ...
      <dl className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {preview.detailRows.map((row) => (
          <div key={row.label} className="rounded-lg border border-stone-200 bg-stone-50 p-4">
            <dt className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">{t(row.label)}</dt>
      ...
      <div className="mt-5 rounded-lg border border-stone-200 bg-white p-4">
        <p className="text-xs font-bold uppercase tracking-[0.16em] text-stone-500">{t('Advisory transition plan')}</p>
        <p className="mt-2 text-sm leading-6 text-stone-600">
          {t('These recommendations are read-only and apply only after future provider success, persistence, audit, and operator approval rules exist.')}
        </p>
      ...
      {warnings.length ? (
        <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-950">
          <p className="text-xs font-bold uppercase tracking-[0.16em]">{t('Preview warnings')}</p>
