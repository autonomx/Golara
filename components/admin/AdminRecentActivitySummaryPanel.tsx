...
export function AdminRecentActivitySummaryPanel({ summary }: { summary: RecentActivitySummary }) {
  return (
    <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-stone-500">Analytics</p>
          <h2 className="mt-1 text-2xl font-bold text-stone-950">Recent activity timeline</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">Unified operational feed from order timeline events, customer timeline events, and admin audit logs.</p>
        </div>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-4">
        <Metric label="Activities reviewed" value={summary.totalActivities} />
        <Metric label="Staff activities" value={summary.staffActivities} />
        <Metric label="System activities" value={summary.systemActivities} />
        <Metric label="Sources" value={summary.bySource.length} />
      </div>
            ...
            <tbody>
              {summary.entries.map((row) => (
                <tr key={row.id} className="border-t border-stone-200">
                  ...
                  <td className="px-3 py-2 text-stone-700">{row.source}</td>
                  <td className="px-3 py-2 text-stone-700">{row.actorLabel}</td>
