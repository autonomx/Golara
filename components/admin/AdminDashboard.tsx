...
export function AdminDashboard({ activeWorkspace, categories, products, homepage, homepageTranslations, media, authEventSummary, runtimeReadiness, authConfigured, authenticated, notificationReadiness, notificationRetryRunbook, checkoutReadiness, catalogSearch, catalogCategory, catalogFlag, status, message }: AdminDashboardProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      {showCatalog ? <section id="media" className={panelClass}><div className="mb-6"><p className="text-sm font-semibold uppercase tracking-[0.25em] text-olive">Media library</p><h2 className="mt-2 font-display text-4xl text-rosewood">Images</h2><p className="mt-3 text-sm leading-6 text-stone-600">Register external image URLs or upload local/dev images into <code>public/uploads</code>.</p></div><div className="grid gap-6 lg:grid-cols-2"><form action={createMediaFromUrlAction} className={formCardClass}><h3 className="font-display text-3xl text-rosewood">Add image URL</h3><Field label="Image URL" name="url" placeholder="https://..." disabled={disabled} /><Field label="Alt text" name="alt" placeholder="Blush rose bouquet" disabled={disabled} /><SubmitButton disabled={disabled}>Add media</SubmitButton></form><form action={uploadMediaAction} className={formCardClass}><h3 className="font-display text-3xl text-rosewood">Upload image</h3><label className="grid gap-2 text-sm font-semibold text-rosewood">Image file<input className={inputClass} name="file" type="file" accept="image/jpeg,image/png,image/webp,image/gif" required disabled={disabled} /></label><Field label="Alt text" name="alt" placeholder="Optional descriptive text" required={false} disabled={disabled} /><SubmitButton disabled={disabled}>Upload image</SubmitButton></form></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{media.map((item) => <article key={item.url} className="overflow-hidden rounded-lg border border-rosewood/10 bg-cream shadow-sm"><div className="relative aspect-square bg-blush"><Image src={item.url} alt={item.alt} fill className="object-cover" sizes="25vw" /></div><div className="space-y-2 p-4"><p className="text-sm font-semibold text-rosewood">{item.alt}</p><MediaMeta item={item} /><p className="break-all text-xs text-stone-500">{item.url}</p></div></article>)}</div></section> : null}
...
function ProductTable({ products, categories, media, disabled }: { products: Product[]; categories: Category[]; media: MediaItem[]; disabled: boolean }) {
  return <div className="mt-4 max-h-[760px] overflow-auto rounded-lg border border-rosewood/10 bg-white [scrollbar-width:thin] [scrollbar-color:#6f2438_#fff8f1]"><table className="w-full min-w-[980px] border-collapse text-left text-sm"><thead className="sticky top-0 z-[1] bg-cream text-xs font-semibold uppercase tracking-[0.16em] text-rosewood/70"><tr><th className="w-12 px-4 py-3">Pick</th><th className="px-4 py-3">Product</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Flags</th><th className="px-4 py-3">Actions</th></tr></thead><tbody>{products.map((product) => <Fragment key={product.slug}><tr className="border-t border-rosewood/10 align-top"><td className="px-4 py-4"><input form="bulk-products-form" type="checkbox" name="productId" value={product.id ?? ''} disabled={disabled || !product.id} /></td><td className="px-4 py-4"><div className="flex gap-3"><div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-blush">{product.image ? <Image src={product.image} alt={product.title} fill className="object-cover" sizes="56px" /> : null}</div><div><div className="font-semibold text-rosewood">{product.title}</div><div className="mt-1 text-xs text-stone-500">{product.code} - {product.slug}</div></div></div></td><td className="px-4 py-4 text-stone-700">{product.categoryTitle || product.category}</td><td className="px-4 py-4 text-stone-700">{product.price} {product.currency}</td><td className="px-4 py-4"><div className="flex flex-wrap gap-2">{product.bestSeller ? <span className="rounded-full bg-rosewood px-2 py-1 text-xs font-semibold text-white">Best</span> : null}{product.availableToday ? <span className="rounded-full bg-olive px-2 py-1 text-xs font-semibold text-white">Today</span> : null}{product.requiresQuote || product.price <= 0 ? <span className="rounded-full bg-stone-800 px-2 py-1 text-xs font-semibold text-white">Quote</span> : null}{product.isActive === false ? <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-700">Inactive</span> : null}</div></td><td className="px-4 py-4"><div className="flex gap-3"><a href={`/products/${product.slug}`} className="text-xs font-semibold text-rosewood underline-offset-4 hover:underline" target="_blank">View</a><a href={`#edit-${product.slug}`} className="text-xs font-semibold text-rosewood underline-offset-4 hover:underline">Edit</a></div></td></tr><tr id={`edit-${product.slug}`} className="border-t border-rosewood/10 bg-[#fffdfb]"><td colSpan={6} className="px-4 py-4"><details><summary className="cursor-pointer font-display text-2xl text-rosewood">Edit {product.title}</summary><form action={updateProductAction.bind(null, product.id ?? '')} className="mt-5 grid gap-4"><ProductFields product={product} categories={categories} media={media} disabled={disabled || !product.id} /><SubmitButton disabled={disabled || !product.id}>Update product</SubmitButton></form></details></td></tr></Fragment>)}</tbody></table></div>;
}
...
function MediaSelect({ label, name, media, defaultValue, disabled }: { label: string; name: string; media: MediaItem[]; defaultValue?: string; disabled: boolean }) {
  return <MediaSelectWithPreview label={label} name={name} media={media} defaultValue={defaultValue} disabled={disabled} className={inputClass} />;
}
