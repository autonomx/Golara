...
export async function runFailedPaymentAlertLocalizationTests() {
  ...
  const paymentAlert = { kind: 'payment', status: 'failed', title: 'manual payment failed' };
  const retryAlert = {
    kind: 'notification',
    status: 'retry_scheduled',
    title: 'EMAIL notification retry scheduled',
