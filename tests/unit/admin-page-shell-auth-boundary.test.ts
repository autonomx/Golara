...
export async function runAdminPageShellAuthBoundaryTests() {
  ...
  assert.ok(shellSource.includes("if (!props.authenticated) redirect('/admin/login');"), 'AdminPageShell must redirect unauthenticated dedicated admin pages to /admin/login before rendering shell content');
  assert.ok(shellSource.indexOf("if (!props.authenticated) redirect('/admin/login');") < shellSource.indexOf('return ('), 'AdminPageShell auth redirect must run before rendering admin navigation or children');
